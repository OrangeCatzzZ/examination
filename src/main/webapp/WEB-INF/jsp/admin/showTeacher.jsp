				...
				<div class="panel panel-default">
				    ...
				    <table class="table table-bordered">
							...
							<c:forEach  items="${teacherList}" var="item">
								<tr>
									<td>${item.teacher.userid}</td>
									<td>${item.teacher.username}</td>
									<td>${item.teacher.sex}</td>
									<td><fmt:formatDate value="${item.teacher.birthyear}" dateStyle="medium" /></td>
									<td>${item.teacher.degree}</td>
									<td>${item.teacher.title}</td>
									<td><fmt:formatDate value="${item.teacher.grade}" dateStyle="medium" /></td>
									<td>${item.collegeName}</td>
									<td>
										<button class="btn btn-default btn-xs btn-info" onClick="location.href='${pageContext.request.contextPath}/admin/editTeacher?id=${item.teacher.userid}'">修改</button>
										<button class="btn btn-default btn-xs btn-danger btn-primary" onClick="location.href='${pageContext.request.contextPath}/admin/removeTeacher?id=${item.teacher.userid}'">删除</button>
										<!--弹出框-->
									</td>
								</tr>
							</c:forEach>
